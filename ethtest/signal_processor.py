...
class SignalProcessor:
    ...
    def fetch_signal(self, key: str = "ETHUSDT_signal") -> Optional[Dict[str, Any]]:
        try:
    ...
    def process_signal(self, signal_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for pos in positions:
                ...
                if pos_symbol and "ETHUSD" in pos_symbol:
                    try:
                    ...
                    if "buy" in signal_text and pos_size < 0:
                        ...
                        self.trade_manager.place_market_order(
                            "ETHUSD", "buy", abs(pos_size), params={"time_in_force": "ioc"}, force=True
                        )
                        time.sleep(2)
                        ...
                        self.trade_manager.place_market_order(
                            "ETHUSD", "sell", pos_size, params={"time_in_force": "ioc"}, force=True
                        )
                        time.sleep(2)
        except Exception as e:
        ...
        self.cancel_conflicting_orders("ETHUSD", "buy" if "buy" in signal_text else "sell")
        self.cancel_same_side_orders("ETHUSD", "buy" if "buy" in signal_text else "sell")
        time.sleep(2)

        if self.order_manager.has_open_position("ETHUSD", "buy" if "buy" in signal_text else "sell"):
            logger.info("An open %s position exists for ETHUSD. Skipping new order placement.",
        ...
        try:
            limit_order = self.order_manager.place_order("ETHUSD", 
                                                        "buy" if "buy" in signal_text else "sell", 
                                                        1, 
                                                        entry_price,
