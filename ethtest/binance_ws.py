...
class BinanceWebsocket:
    def __init__(self, stream_url="wss://fstream.binance.com/ws", subscribe_params=["ethusdt@aggTrade"], reconnect_interval=10):
        self.logger = logging.getLogger(self.__class__.__name__)
        self.stream_url = stream_url
        self.subscribe_params = subscribe_params  # Now uses ETH/USDT stream by default.
        ...
        while True:
            with ws._lock:
                if ws.current_price is not None:
                    print(f"Latest ETH/USDT price: {ws.current_price}")
            time.sleep(2)
