...
class ProfitTrailing:
    ...
    def set_zone_limits(
        ...
        # DEBUG INFO
        logger.info(f"[DEBUG] set_zone_limits -> target_long={self.target_long}, target_short={self.target_short}")
    ...
    def update_trailing_stop(
        ...
        # DEBUG INFO
        logger.info(
            f"[DEBUG] update_trailing_stop -> entry={entry}, size={size}, "
            f"target_long={self.target_long}, target_short={self.target_short}, live_price={live_price}"
        )

        # compute and store max profit
    ...
    def track(self) -> None:
        ...
        while True:
            ...
            for pos in self.cached_positions:
                ...
                # consistent key for max_profit lookup
                pos_symbol = pos.get('info', {}).get('product_symbol') or pos.get('symbol', 'unknown')
                entry_val = pos.get('info', {}).get('entry_price') or pos.get('entryPrice')
                size_val = pos.get('size') or pos.get('contracts')
                key = f"{pos_symbol}_{entry_val}_{size_val}"
